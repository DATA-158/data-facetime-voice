...
# "Thinking" sound while a turn has produced no speech yet (the LLM's first
# token is the one latency we can't shorten). Captain's pick 2026-09-12: a
# warm hum — a quiet C-G-C chord that swells from silence to peak and back
# over THINK_HUM_S, then THINK_GAP_S of silence. (A fast 0.6 s beep was tried
# first and rejected as alarm-like.) Pushed in 100 ms slices just-in-time so
# at most ~150 ms is ever queued, and flushed with CLEAR the instant the first
# sentence is ready — it never delays DATA. Pauses under the filler/stall
# line; stops on speech or barge-in; not counted as speech for VAD.
THINKING_SOUND = os.environ.get("DFV_THINKING_SOUND", "hum").lower()    # hum | off
...
class AudioSession:
    ...
    def play_raw(self, pcm: np.ndarray):
        """Queue audio WITHOUT counting it as speech (thinking hum): VAD
